...
public class MsoVnfCloudifyAdapterImpl implements MsoVnfAdapter {
    ...
    public void rollbackVnf (VnfRollback rollback) throws VnfException {
        ...
        try {
            ...
            deployment = cloudifyUtils.uninstallAndDeleteDeployment(cloudSiteId, tenantId, vfModuleId, 5);
        ...
        throws VnfException
    {
        ...
        // Obtain the cloud site information where we will create the VF Module
           Optional<CloudSite> cloudSite = null;
           try{
             cloudSite = cloudConfigFactory.getCloudConfig().getCloudSite(cloudSiteId);
           }
           catch(Exception e ){
             LOGGER.debug("EXCEP:"+e.getMessage());
             throw e;
           }
             LOGGER.debug("CLOUD SITE IS NULL? = "+String.valueOf(cloudSite == null));
        if (!cloudSite.isPresent()) {
             LOGGER.debug("NO CLOUD SITE");
