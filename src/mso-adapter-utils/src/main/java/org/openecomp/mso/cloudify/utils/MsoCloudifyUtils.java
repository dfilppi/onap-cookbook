...
public class MsoCloudifyUtils extends MsoCommonUtils implements VduPlugin{
    ...
    {
        LOGGER.debug("entering createAndINstallDeployment");
        ...
        LOGGER.debug("getting cloudify client");
...
//    		LOGGER.error (MessageEnum.RA_CREATE_STACK_ERR, "Execute Workflow (" + command + "): Unexpected error: " + e, "", "", MsoLogger.ErrorCode.BusinessProcesssError, "Execute Workflow (" + command + "): Internal error" + e.getMessage());
    	...
    	throws MsoException
    {
        ...
        try{
           cloudify = getCloudifyClient (cloudSite.get());
        }
        catch(Exception e){
           StackTraceElement[] els = e.getStackTrace();
           for(StackTraceElement el:els){
             LOGGER.debug(String.format("--> %s [ %s:%d ]",el.getClassName(), el.getMethodName(), el.getLineNumber()));
           }
...
//    		deployment = queryDeployment.execute();
    		...
    		// Catch-all
        LOGGER.debug ("CATCH ALL:"+e.getMessage());
    		throw new MsoAdapterException (e.getMessage(), e);
    	}
    }
    ...
    {
        LOGGER.debug("CloudSite = "+cloudSite);
        ...
        LOGGER.debug("CloudifyURL = "+cloudifyUrl);
        Cloudify cloudify = new Cloudify (cloudifyUrl);
        LOGGER.debug("after new cloudify");
        ...
        LOGGER.debug("after set token provider");
    ...
    protected OpenstackConfig getOpenstackConfig (CloudSite cloudSite, String tenantId) {
        ...
        openstackConfig.setTenantId (tenantId);
